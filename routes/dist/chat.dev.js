...
router.post("/", function (req, res) {
  ...
  console.log("Amigo: " + req.session.amigoid);
...
router.post("/recebemensagens", function (req, res) {
  ...
  var sql = "INSERT INTO chats (enviou_id, recebeu_id, mensagem, lida) VALUES ?";
...
router.post("/buscamensagens", function (req, res) {
  ...
  con.query(sql, amigo, function (err, result, fields) {
    ...
    valores = [usuario_logado, amigo, amigo, usuario_logado]; //sql5 = "SELECT COUNT(id) FROM `chats` WHERE lida=0 && recebeu_id=?"
    //valores = [usuario_logado];

    sql2 = "SELECT * FROM chats WHERE (enviou_id=? && recebeu_id= ?) or (enviou_id=? && recebeu_id= ?) ORDER BY id  LIMIT 500;";
    con.query(sql2, valores, function (err, mensagens, fields) {
      ...
      mensagens.forEach(function (dados) {
        if (usuario_logado == dados["enviou_id"]) {
          retorno = retorno + "<div class='media media-chat media-chat-reverse'>" + "<img class='avatar' src=imagens/" + foto_logado + ">" + "<div class='media-body'>" + "<p>" + dados["mensagem"] + "</p>" + "</div>" + "</div>" + "<div class='media media-meta-day'> </div>";
        } else {
          retorno = retorno + "<div class='media media-chat'>" + "<img class='avatar' src=imagens/" + foto_amigo + ">" + "<div class='media-body'>" + "<p>" + dados["mensagem"] + "</p>" + "</div>" + "</div>" + "<div class='media media-meta-day'> </div>";
        }
      });
